...
def merge_copy_db(db_path, save_path):
    ...
    if not os.path.exists(db_path):
        info = f'db_path: {db_path} dir not found!'
...
def merge_msg_db(db_path: list, save_path: str, create_time: int = 0):  # CreateTime: 从这个时间开始的消息 10位时间戳
    # 判断save_path是否为文件夹
        ...
        for tbl_name in tabels_all:
            if tbl_name == 'MSG':
                MsgSvrIDs = merged_cursor.execute(
                    f'select MsgSvrID from MSG where CreateTime>{create_time} and MsgSvrID!=0').fetchall()

                cursor.execute(f'PRAGMA table_info({tbl_name})')
                columns = cursor.fetchall()
                columns = [column[1] for column in columns[1:]]

                ex_sql = f"select {','.join(columns)} from {tbl_name} where CreateTime>{create_time} " \
                         f"and MsgSvrID not in ({','.join([str(MsgSvrID[0]) for MsgSvrID in MsgSvrIDs])})"
                cursor.execute(ex_sql)

                insert_sql = f"INSERT INTO {tbl_name} ({','.join(columns)}) VALUES " \
                             f"({','.join(['?' for _ in range(len(columns))])})"
                try:
...
def merge_media_msg_db(db_path: list, save_path: str):
    ...
    for db_file in db_path:
        ...
        if len(have_tables) < 1:
            ...
            sql = 'CREATE TABLE Media(localId INTEGER  PRIMARY KEY AUTOINCREMENT,Key TEXT,Reserved0 INT,Buf BLOB,' \
        ...
        for tblname in have_tables:
            ...
            ex_sql = f"select `Key`,Reserved0,Buf,Reserved1,Reserved2 from {tblname} where Reserved0 not in " \
                     f"({','.join([str(r[0]) for r in r0])}) "
            cursor.execute(ex_sql)
            data = cursor.fetchall()

            insert_sql = f"INSERT INTO {tblname} (Key,Reserved0,Buf,Reserved1,Reserved2) VALUES " \
                         f"({','.join(['?' for _ in range(5)])})"
            try:
...
def merge_db(db_paths, save_path='merge.db', create_time: int = 0):
    if os.path.isdir(save_path):
    ...
    with sqlite3.connect(save_path) as outdb:
        ...
        for alias in databases:
            ...
            for table in tables:
                ...
                # 获取表中的数据
                if 'CreateTime' in columns and create_time > 0:
                    sql = f'SELECT * FROM {table} WHERE CreateTime>? ORDER BY CreateTime'
                    src_data = execute_sql(db, sql, (create_time,))
                else:
